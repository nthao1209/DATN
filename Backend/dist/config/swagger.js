...
const getFirstOperation = (pathItem) => methodOrder
    .map((method) => pathItem[method])
    .find((operation) => operation);
const sortSwaggerPaths = (spec) => {
    const paths = spec.paths || {};
    const sortedPaths = Object.entries(paths).sort(([pathA, pathItemA], [pathB, pathItemB]) => {
        const operationA = getFirstOperation(pathItemA);
        const operationB = getFirstOperation(pathItemB);
        const tagDiff = getTagIndex(operationA?.tags?.[0]) - getTagIndex(operationB?.tags?.[0]);
        if (tagDiff !== 0)
            return tagDiff;
        return pathA.localeCompare(pathB);
    });
    spec.paths = Object.fromEntries(sortedPaths.map(([path, pathItem]) => {
        const sortedMethods = Object.entries(pathItem).sort(([methodA], [methodB]) => methodOrder.indexOf(methodA) - methodOrder.indexOf(methodB));
        return [path, Object.fromEntries(sortedMethods)];
    }));
    return spec;
};
exports.swaggerSpec = sortSwaggerPaths((0, swagger_jsdoc_1.default)({
    definition: {
        openapi: '3.0.0',
        info: {
    ...
    apis: [
        ...
        path_1.default.join(__dirname, '../../src/routes/**/*.ts'),
    ],
}));
