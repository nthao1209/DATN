...
async function init() {
    ...
    const createNotification = async (userId, type, title, content, payload) => {
    ...
    const handleAttendanceMessage = async (_topic, data) => {
        try {
            ...
            const candidateIn = data.checkInBy ??
                data.user ??
                data.operator;
            const candidateOut = data.checkOutBy ??
                data.user ??
                data.operator;
            ...
            const checkOutStatusChanged = isNewTransaction
                ...
                : Boolean(existing.checkOut) !==
                    Boolean(data.checkOut);
            const hasAttendanceStatusChanged = checkInStatusChanged ||
            ...
            const resolveEventBusIdByActor = async (actorId, fallbackBusId) => {
                if (!actorId || !tripId) {
                    return fallbackBusId;
                }
                const actorBusRes = await pool.query(`
                            SELECT id
                            FROM "Bus"
                            WHERE "tripId" = $1
                              AND "managerId" = $2
            ...
            const checkOutBusRes = await pool.query(`
                ...
                WHERE id = $1
                `, [eventCheckOutBusId]);
            const checkInBusCode = checkInBusRes.rows[0]?.busCode ||
                eventCheckInBusId;
            const checkOutBusCode = checkOutBusRes.rows[0]?.busCode ||
                eventCheckOutBusId;
            const autoCheckInNote = passengerInfo &&
                Number(passengerInfo.busId) !==
                    Number(eventCheckInBusId)
                ? `Khách đang ở trên xe ${checkInBusCode}`
                : null;
            const autoCheckOutNote = passengerInfo &&
                Number(passengerInfo.busId) !==
            ...
            const isMisassigned = passengerInfo &&
                Number(passengerInfo.busId) !==
                    Number(latestEventBusId);
            const assignedManagerId = passengerInfo?.managerId ?? null;
            const isWrongBus = Boolean(isMisassigned);
            const targetManagerId = assignedManagerId;
            if (hasAttendanceStatusChanged &&
                isWrongBus &&
                targetManagerId) {
                const content = `Khách ${passengerInfo?.name ||
                    `#${result.passengerId}`} của xe ${passengerInfo?.busCode ||
                    passengerInfo?.busId} vừa được điểm danh trên xe ${actualBus?.busCode ||
                    latestEventBusId} ở chặng ${roundInfo?.name ||
                    result.roundId}.`;
                await createNotification(targetManagerId, 'attendance.wrong_bus', 'Khách sai xe', content, {
                    tripId,
