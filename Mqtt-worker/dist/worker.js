...
async function init() {
    ...
    const createNotification = async (userId, type, title, content, payload) => {
        if (!Number.isInteger(userId) || userId <= 0)
            return;
        const exists = await pool.query(`SELECT id FROM "Notification"
             WHERE "userId" = $1
               AND type = $2
               AND (payload->>'transactionId')::int = $3
               AND (payload->>'passengerId')::int = $4
             ORDER BY "createdAt" DESC
             LIMIT 1`, [userId, type, Number(payload.transactionId || 0), Number(payload.passengerId || 0)]);
        if (exists.rows.length > 0) {
            return;
        }
        await pool.query(`INSERT INTO "Notification" ("userId", type, title, content, payload, "isRead", "createdAt")
             VALUES ($1, $2, $3, $4, $5, false, NOW())`, [userId, type, title, content, JSON.stringify(payload)]);
    };
    client.on('connect', () => {
    ...
    const handleAttendanceMessage = async (_topic, data) => {
        try {
            ...
            let result = null;
            if (existing) {
                const updateRes = await pool.query(`UPDATE "Transaction" SET "busId" = $1, "checkIn" = $2, "checkOut" = $3, "checkInNote" = $4, "checkOutNote" = $5 WHERE id = $6 RETURNING *`, [
                    data.busId,
                ...
                const insertRes = await pool.query(`INSERT INTO "Transaction" ("passengerId", "roundId", "busId", "checkIn", "checkOut", "checkInNote", "checkOutNote") VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`, [
                    data.passengerId,
                    data.roundId,
                    data.busId,
                    Boolean(data.checkIn),
            ...
            if (!tripId) {
                if (process.env.NODE_ENV === 'development')
                    parentPort?.postMessage(`⚠️ [${prj}] Could not determine tripId for bus ${result.busId}, skipping realtime publish`);
                return;
            }
